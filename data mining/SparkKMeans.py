...
data = sc.parallelize(scale(createClusteredData(1000, K)))
...
points = array(data.collect())
plt.scatter(points[:,0], points[:,1], c=array(results).astype(float))

centroids = clusters.centers

plt.scatter(array(centroids)[:, 0], array(centroids)[:, 1],c="r", marker="x")
plt.savefig("./data mining/plot/kmeanspark.png", format="png")
